...
export const getHistoriesByUser = async (user : User): Promise<History[]> => {
  ...
  try {
    const response: AxiosResponse<History[]> = await historyServiceAxiosInstance.get("/", {
      data: user,
    });
...
export const createHistory = async (history: History): Promise<History> => {
  try {
    const response: AxiosResponse<History> = await historyServiceAxiosInstance.post("/", history, {
    });
